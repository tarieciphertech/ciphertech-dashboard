...
function ForgotPassword() {
  ...
  return <div className="auth-shell"><div className="auth-grid"/><form className="login-card" onSubmit={submit}>
    ...
    {!isSupabaseConfigured&&<div className="alert warning">Authentication is not configured. Add the Supabase environment variables before deployment.</div>}
    {error&&<div className="alert danger">{error}</div>}
    {sent&&<div className="alert success">If an account exists for this email, a password reset email has been sent. Check your inbox and follow the secure link.</div>}
...
function ResetPassword() {
  ...
  if (!session) return <div className="auth-shell"><div className="auth-grid"/><div className="login-card">
    <div className="brand-mark"><span>⚡</span> CYPHER TECHNOLOGIES</div><p className="eyebrow">ACCOUNT RECOVERY</p><h1>Reset link required</h1><p className="muted">Open the password reset link from your Cypher Technologies email to continue.</p><NavLink to="/forgot-password" className="primary full">Request another link <ChevronRight size={17}/></NavLink>
  </div></div>
  return <div className="auth-shell"><div className="auth-grid"/><form className="login-card" onSubmit={submit}>
    <div className="brand-mark"><span>⚡</span> CYPHER TECHNOLOGIES</div><p className="eyebrow">ACCOUNT RECOVERY</p><h1>Set a new password</h1><p className="muted">Choose a strong password for your Cypher Technologies admin account.</p>
    {error&&<div className="alert danger">{error}</div>}{done&&<div className="alert success">Password updated. Redirecting to the admin portal…</div>}
    ...
    <label>Confirm new password<input value={confirm} onChange={e=>setConfirm(e.target.value)} type="password" autoComplete="new-password" minLength={8} required/></label>
    <button className="primary full" disabled={busy||done}>{busy?'Updating…':'Set new password'} <ChevronRight size={17}/></button>
    <div className="security-note"><ShieldCheck size={16}/> Your password is managed securely by Supabase Auth.</div>
  </form></div>
}
